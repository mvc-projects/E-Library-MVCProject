...
namespace MVCProjectVIdent.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
           // var user = _userManager.Users.FirstOrDefault(c => c.Id == User.Identity.GetUserId());
        ...
        [Authorize]
        [HttpPost]
        public ActionResult profile(ApplicationUser applicationUser)
        {
            // var user = _userManager.Users.FirstOrDefault(c => c.Id == User.Identity.GetUserId());
            var user = db.Users.Find(User.Identity.GetUserId());
		...
		public ActionResult uploadImage(HttpPostedFileBase image)
        {
	        ...
	        {
		        byte[] imageBytes;
		        using (var binaryData = new BinaryReader(image.InputStream))
		        {
			        imageBytes = binaryData.ReadBytes(image.ContentLength);
		        }
