...
namespace MVCProjectVIdent.Controllers
{
	...
	public class AdminEmployeeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            return View(db.Users.Where(n => !n.isDeleted && n.Roles.Select(x => x.RoleId).Contains(db.Roles.FirstOrDefault(c => c.Name == MyRole.Employee).Id)).ToList());
        }
        ...
        public async Task<ActionResult> Create([Bind(Include = "Email,Password,FirstName,LastName,Salary,HireDate")] AdminModel adminModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    await um.AddToRoleAsync(user.Id, MyRole.Employee);
        ...
        public ActionResult Remove(string id)
        {
	        ...
	        var notify2 = new Notification()
	        {
		        ...
		        Message = $"User : {user.UserName} remove By {User.Identity.GetUserName()} ",
		        Title = "remove Data"
	        };
